...
with st.sidebar:
    ...
    target_vibe = st.text_input("目标风格", placeholder="例如：赛博朋克，高级灰")
    
    go_btn = st.button("开始分析", type="primary", use_container_width=True)
...
with col2:
    ...
    if go_btn and uploaded_file and api_key:
        try:
            ...
            # --- 关键修改：换成了 Flash 模型，更稳 ---
            model = genai.GenerativeModel('gemini-1.5-flash')
            
            with st.spinner("⚡️ AI 正在极速分析中..."):
                prompt = f"""
            ...
            st.error(f"发生错误: {e}")
            st.caption("建议检查 API Key，或确保网络能访问 Google 服务。")
    elif go_btn and not api_key:
        st.warning("⚠️ 别忘了填入 API Key！")
